...
def main(idx):
    if not os.path.exists(f"/scratch/nf33/hk25_LSP/UK/MC/cape_cin/cape_cin_time{idx:04d}.nc"):
        # load only one time step
        ds_t = xr.open_dataset(
            "/scratch/nf33/hk25_LSP/UK/MC/ta_3hourly.nc").isel(time=idx)
    
        ds_tdew = xr.open_dataset(
            "/scratch/nf33/hk25_LSP/UK/MC/tdew_3hourly.nc").isel(time=idx)

        ds_tas = xr.open_dataset("/scratch/nf33/hk25_LSP/UK/MC/tas_3hourly.nc").isel(time=idx)
        ta_surf = ds_tas["tas"].stack(ngrid=('latitude','longitude')).values.flatten()-273.15
        
        ds_tdews = xr.open_dataset("/scratch/nf33/hk25_LSP/UK/MC/tdews_3hourly.nc").isel(time=idx)
        ...
        psurf[:] = 1031
        plvl = ds_t.pressure.isel(pressure=slice(None, None, -1)).values.astype(float)
        ...
        out.to_netcdf(f"/scratch/nf33/hk25_LSP/UK/MC/cape_cin/cape_cin_time{idx:04d}.nc")
